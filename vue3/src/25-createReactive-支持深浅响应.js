...
// 问题 j因为建立的是浅响应关系，导致在设置bar属性值的时候，不会触发副作用函数
// 方案： 递归调用reactive函数，建立关系
obj.foo.bar = 2

effect(() => {
  console.log(obj2.profile.name)
})
// 因为是shallowReactive所以不会触发副作用函数
obj2.profile.name = 'change name'
