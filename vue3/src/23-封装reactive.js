...
// 问题，当触发修改操作时，会触发两次副作用函数
// 因为child自身没有bar属性，会在原型链上寻找bar属性，因此child和parent都会建立跟副作用函数的关系
// 设置的属性如果不存在雨对象上，那么会取得其原型，并调用原型的set方法，所以child.bar = 2会触发两次副作用函数
// 方案： 屏蔽触发parent的set拦截函数

// child的set拦截函数
// set(target, key, value, receiver){
//   // target 是原始对象obj
//   // receiver 是代理对象child child是obj的代理对象
// }

// parent的set拦截函数
// set(target, key, value, receiver){
//   // target 是原始对象proto
//   // receiver 是代理对象child child 不是 proto的代理对象
// }

// 总结： receiver就是代理对象，如child， parent，只有当 receiver 是 target 的代理对象时才触发更新，这样就能够屏蔽由原型引起的更新了。
// 设计：给get拦截器设计一个raw属性，如果是raw属性，返回target
