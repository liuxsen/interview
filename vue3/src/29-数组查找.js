...
const arrayInstrumentations = {
  // 因为在includes中传参有可能是代理对象的元素也可能是原始数组对象的元素，所以需要重写方法，两次查找元素
  includes: function(...args){
    // this是代理对象
    let res = originMethod.apply(this, args)
    if(res === false){
      // res为false值，说明没找到，通过this.raw 拿到原始数组对象，再去其中查找并更新res值
...
effect(() => {
  ...
  // 因为arr[0] 返回了一个新的proxy对象
  console.log(arr.includes(arr[0]))
  // 通过重写数组includes方法，可以查找一次原始对象
  console.log(arr.includes(data[0]))
})
