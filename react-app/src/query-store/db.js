...
export const updatePost = async (id, post) => {
  await api()
  const res = db.posts.map(item => {
    return item.id === id ? {...item, ...post} : item
  })
  return res
}

export const delPost = async (id, post) => {
  await api()
  const index = db.posts.findIndex(item => {
    return item.id === id
  })
  if(index > 0){
